...
import { Pair } from '@myunidavid/v2-sdk/dist/entities';
...
import { FeeAmount, Pool } from '@myunidavid/v3-sdk';
...
export async function getHighestLiquidityV3USDPool(
  ...
  const usdPools = _([
    ...
    .value();
  // console.log(usdPools);
  const poolAccessor = await poolProvider.getPools(usdPools, providerConfig);
  const pools: number[] = _([
    FeeAmount.HIGH,
    FeeAmount.MEDIUM,
    FeeAmount.LOW,
    FeeAmount.LOWEST,
  ...
  const maxPool: any = pools.reduce((prev: any, current: any) => {
    return JSBI.greaterThan(prev.liquidity, current!.liquidity)
      ? prev
      : current;
  });
